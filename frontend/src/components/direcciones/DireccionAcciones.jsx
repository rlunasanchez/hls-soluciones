...
function DireccionAcciones({ direccion, onVer, onEditar, onEliminar }) {
    ...
    <div className="acciones-menu" style={{ position: "relative" }}>
      ...
      {abierto && (
        <div className="acciones-dropdown" ref={menuRef} style={{ position: "fixed", top: pos.top, left: pos.left }}>
          <button className="acciones-item ver" onClick={() => { setAbierto(false); onVer(direccion); }}>
            <Eye size={14} /> Ver
          </button>
          <button className="acciones-item edit" onClick={() => { setAbierto(false); onEditar(direccion); }}>
            <Edit size={14} /> Editar
          </button>
          <button className="acciones-item delete" onClick={() => { setAbierto(false); onEliminar(direccion.id); }}>
...
export default DireccionAcciones;
