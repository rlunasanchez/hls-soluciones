...
function UsuarioAcciones({ usuario, onEditar, onResetearPassword, onEliminar }) {
    ...
    <div className="acciones-menu" style={{ position: "relative" }}>
      ...
      {abierto && (
        <div className="acciones-dropdown" ref={menuRef} style={{ position: "fixed", top: pos.top, left: pos.left }}>
          <button className="acciones-item edit" onClick={() => { setAbierto(false); onEditar(usuario); }}>
            <Edit size={14} /> Editar
          </button>
          <button className="acciones-item reset" onClick={() => { setAbierto(false); onResetearPassword(usuario.id); }}>
            <Key size={14} /> Reset
          </button>
          {usuario.usuario !== 'admin' && (
            <button className="acciones-item delete" onClick={() => { setAbierto(false); onEliminar(usuario.id); }}>
