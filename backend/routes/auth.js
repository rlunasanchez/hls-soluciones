  ...
  try {
    ...
    await pool.query(
      `INSERT INTO usuarios (usuario, password, rol, email, activo) 
       ...
       ON CONFLICT (usuario) DO UPDATE SET password = EXCLUDED.password, rol = 'admin', activo = true`,
...
router.put("/activar-usuario/:id", authMiddleware, adminOnly, async (req, res) => {
  ...
  try {
    ...
    if (result.rows[0].usuario === "admin" && activo === false) {
...
router.delete("/eliminar-usuario/:id", authMiddleware, adminOnly, async (req, res) => {
  try {
    const result = await pool.query("SELECT usuario FROM usuarios WHERE id = $1", [req.params.id]);
    if (result.rows.length === 0) {
      return res.status(404).json({ msg: "Usuario no encontrado" });
    }
    if (result.rows[0].usuario === "admin") {
