...
router.get("/", authMiddleware, async (req, res) => {
  ...
  try {
    const ordenes = await pool.query(
      "SELECT * FROM ordenes_trabajo ORDER BY id DESC LIMIT $1 OFFSET $2",
      [limit, offset]
    );
...
router.get("/siguiente-numero", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      "SELECT numero_orden FROM ordenes_trabajo WHERE numero_orden LIKE $1 ORDER BY numero_orden DESC LIMIT 1",
      [`OT-${year}-%`]
    );
    let siguiente = 1;
...
router.get("/:id", authMiddleware, async (req, res) => {
  const { id } = req.params;
  try {
    const result = await pool.query("SELECT * FROM ordenes_trabajo WHERE id = $1", [id]);
    if (result.rows.length === 0) {
...
router.post("/", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await client.query(
      ...
      [
        numeroOrden, toDatePg(fecha), esGarantia || false,
        toDatePg(fechaIngreso), fechaIngresoCheck || false, toDatePg(fechaTermino), fechaTerminoCheck || false,
        toDatePg(fechaEntrega), fechaEntregaCheck || false, toDatePg(fechaCompra), fechaCompraCheck || false,
