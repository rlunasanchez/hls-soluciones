...
// ── Distribución: 40 equipos para 30 clientes (algunos clientes con 2 equipos) ──
// Clientes 1-20: 1 equipo cada uno = 20
// Clientes 21-30: 2 equipos cada uno = 20
// Total: 40
const distribucion = [];
  ...
  try {
    ...
    for (let i = 0; i < clientes.length; i++) {
      ...
      await connection.query(
        `INSERT INTO clientes (codigo, razon_social, giro, rut, direccion, ciudad, comuna, telefono, contacto_nombre, contacto_email, contacto_fono, activo)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`,
        [codigo, c.razon_social, c.giro, c.rut, c.direccion, c.ciudad, c.comuna,
         c.telefono, c.contacto, c.email, c.fono]
      );
    }
    const clientesInsertados = clientes.length;
    console.log(`  ${clientesInsertados} clientes insertados`);
    ...
    for (let i = 0; i < equiposDef.length; i++) {
      ...
      const clienteId = distribucion[i];
      const serie = `${e.marca.substring(0, 3).toUpperCase()}-${String(1000 + i)}-${String(clienteId).padStart(2, "0")}`;
      await connection.query(
        `INSERT INTO equipos (codigo, cliente_id, equipo, modelo, marca, serie, contador_pag, nivel_tintas, activo)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)`,
    ...
    console.log(`  ${equiposDef.length} equipos insertados`);
    ...
    for (let i = 0; i < equiposDef.length; i++) {
      ...
      const clienteIdx = distribucion[i] - 1;
      ...
      await connection.query(
        `INSERT INTO ordenes_trabajo (numero_orden, fecha, es_garantia, fecha_ingreso, fecha_ingreso_check,
        ...
        [numOrden, fechaStr, esGarantia,
         ...
         `${e.marca.substring(0, 3).toUpperCase()}-${String(1000 + i)}-${String(clienteIdx + 1).padStart(2, "0")}`,
         String(Math.floor(Math.random() * 50000) + 1000),
         ["Alto", "Medio", "Bajo", "Crítico"][Math.floor(Math.random() * 4)],
         averia, clienteIdx + 1, i + 1]
      );
    }
    console.log(`  ${equiposDef.length} órdenes insertadas`);
