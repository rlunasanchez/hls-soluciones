...
| 2.03 | 20 Agosto 2026 | **Adjuntar imagen o PDF en OT** (`frontend/src/components/ordenes/OrdenFormCliente.jsx`): el input de adjuntos ahora acepta `image/*,.pdf` (antes solo imágenes). El filtro valida tipos `image/` y `application/pdf` (máx. 2 archivos, 5MB c/u). En la lista, el PDF muestra un icono `FileText` rojo en vez de miniatura; las imágenes siguen con thumbnail. El modal "Ver" muestra el PDF en un `<iframe>` a tamaño cómodo de lectura (90vw × 88vh, máx 1000px) y las imágenes como antes. Labels del botón: "Agregar archivo" (tanto vacío como con un archivo). Sin cambios de BD ni backend. Solo frontend |
