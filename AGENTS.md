...
| 1.83 | 18 Agosto 2026 | **Alto estándar 44px para todos los botones en mobile** (`index.css`): regla global en media query ≤768px que aplica `min-height: 44px` + centrado (`align-items/justify-content: center`) a los botones de formularios (`.cf-btn-p/-c/-a/-d`, `.ef-btn-p/-c`, `.of-btn-p/-c`, `.uf-btn-p/-c`) y de mantenedores (`.main-btn`, `.btn-nuevo-cliente`, `.btn-limpiar-equipos`). Antes los de Clientes eran más bajos que los de OT. Solo frontend |
| 1.84 | 18 Agosto 2026 | **Ajuste de altos en mobile**: los botones **"+ Agregar"** (`.cf-btn-a`) y **"Eliminar"** (`.cf-btn-d`) salen de la regla global de 44px → vuelven a su tamaño natural (~26px), igual que el toggle "Ver todas/Ver menos" (`.cf-btn-toggle-suc`). El resto de botones (`.cf-btn-p/-c`, `.ef-btn-p/-c`, `.of-btn-p/-c`, `.uf-btn-p/-c`, `.main-btn`, `.btn-nuevo-cliente`, `.btn-limpiar-equipos`) bajan de `min-height: 44px` a **40px** (los 44px se veían muy grandes). Solo frontend |
| 1.85 | 18 Agosto 2026 | **Botones de acción del form OT con 3 opciones** (`OrdenTrabajo.jsx` + `OrdenTrabajo.css`): al **editar** una OT ahora hay **"Guardar Cambios"** (nuevo, botón verde `.of-btn-s` con `var(--success)`) que guarda y **se mantiene en la OT** recargando datos frescos del API (vuelve a llamar `editarOrden` con la respuesta de `GET /:id`), **"Cerrar"** (azul, guarda y redirige al menú como hacía "Guardar Orden") y **"Cancelar"** (gris, cierra sin guardar). Al **crear** OT nueva se mantiene "Guardar Orden" + "Cancelar". `guardarOrden(e, mantener=false)` acepta parámetro: si `mantener && editingId` no cierra el form. `.of-btn-s` incluido en la regla global de alto 40px en mobile (`index.css`). Solo frontend |
| 1.86 | 18 Agosto 2026 | **Fix duplicados en "Agregar dirección del cliente"** (`OrdenFormCliente.jsx`): el select ofrecía siempre todas las direcciones del cliente, así que se podía agregar la misma dirección múltiples veces. Ahora se filtran con `direccionesDisponibles` (excluye las ya presentes en `direccionesExtra`, comparando por dirección en mayúsculas) y solo se agregan las disponibles. El select **sigue visible** siempre que el cliente tenga direcciones (no desaparece), pero las ya agregadas no aparecen como opción; si ya se agregaron todas, el select queda visible pero `disabled`. Solo frontend |
| 1.87 | 18 Agosto 2026 | **Fix duplicados en "Agregar contacto del cliente"** (`OrdenFormCliente.jsx`): mismo comportamiento que v1.86 pero para contactos. Nuevo `contactosExtraDisponibles` que excluye los contactos ya presentes en `contactosExtra` (comparando por nombre en mayúsculas). El select **sigue visible** si el cliente tiene contactos, las ya agregadas desaparecen de las opciones y se deshabilita cuando no quedan disponibles. Solo frontend |
...
| 1.94 | 20 Agosto 2026 | **Apartado "Adjunto" en form OT** (`OrdenFormCliente.jsx` + `OrdenTrabajo.jsx` + `backend/routes/ordenes.js` + `crear_tablas.sql`): debajo de "Información Interna", nueva sección azul (colapsable con flecha, oculta por defecto) para adjuntar **hasta 2 imágenes** (JPG, PNG, etc.). Se guardan en **Base64** como **JSON array** `[{nombre, tipo, data}]` en la columna `adjunto LONGTEXT` de `ordenes_trabajo` (misma calidad original, se reconstruye exacta al descargar). **Migración local:** `ALTER TABLE ordenes_trabajo ADD COLUMN adjunto LONGTEXT AFTER info_interna;`. **Frontend:** input oculto estilizado como label `.ot-search` ("Seleccionar imagen..." / "Agregar segunda imagen..."), miniaturas con vista previa, menú de 3 puntos por imagen (`.acciones-*`, posicionado con `position:fixed` para no recortarse) con **Ver** (modal grande), **Descargar** (nombre original) y **Eliminar** (con confirmación). Backend POST/PUT reciben `adjuntos` y lo guardan como JSON. Máximo 5MB por imagen. |
| 1.95 | 20 Agosto 2026 | **Fix error 413 Request Entity Too Large al guardar imagen en producción** (`backend/server.js`): `express.json()` y `express.urlencoded()` ahora aceptan hasta **20MB** (antes el default de ~100KB). Necesario porque las imágenes en Base64 agrandan ~33% el tamaño y 2 imágenes de 5MB pueden sumar ~13MB en el body JSON. Ambas ramas (`main` y `deploy/cloud`). |
| 1.96 | 20 Agosto 2026 | **Mensaje de advertencia si las imágenes adjuntas suman más de 18 MB** (`frontend/src/pages/OrdenTrabajo.jsx`): en `guardarOrden`, antes de enviar la OT al servidor, calcula el tamaño total aproximado de los adjuntos (data URL base64 → bytes reales × 0.75). Si supera 18 MB (cerca del límite de 20 MB del servidor), muestra `alert` con el peso exacto y no envía la orden, permitiendo al usuario reducir o eliminar imágenes antes de guardar. Solo frontend |
| 1.97 | 20 Agosto 2026 | **Validación de campos obligatorios en el mantenedor de Equipos** (`backend/routes/equipos.js` + `frontend/src/components/equipos/EquipoFormulario.jsx` + `frontend/src/pages/Equipos.jsx`): POST y PUT de `/api/equipos` rechazan con 400 si Equipo, Marca o Modelo vienen vacíos (`"Complete Equipo, Marca y Modelo antes de guardar"`). Frontend: alerta en `handleSubmit` antes de guardar si falta alguno. `Equipos.jsx` muestra `err.response?.data?.msg` en vez de "Error al guardar" genérico. **La serie NO es única ni obligatoria**: se pueden crear equipos con la misma serie o sin serie. Solo `main` por ahora |
| 1.98 | 20 Agosto 2026 | **Botón "Registrar en Equipos" en el form OT** (`frontend/src/pages/OrdenTrabajo.jsx` + `frontend/src/components/ordenes/OrdenFormEquipo.jsx`): en la sección "Datos del Equipo", botón verde "+ Registrar en Equipos" que aparece **solo al crear una OT nueva** (`!readOnly && !editingId`). Al clickear, `registrarEquipoEnMantenedor()` valida que Equipo/Marca/Modelo estén completos (la serie es opcional), llama a `POST /api/equipos` y: si los datos son válidos → crea el equipo en el mantenedor, alerta "Equipo creado con código EQ-XXXX" y refresca el array `equipos` local; si falta algún campo obligatorio → alerta "Complete Equipo, Marca y Modelo". En todos los casos **te quedás en la OT** (no navega ni cierra). No aparece en editar/ver. Solo `main`, sin cambios de BD |
| 1.99 | 20 Agosto 2026 | **Quitar regla de serie única + validación de campos obligatorios al crear Equipos** (ambas ramas): eliminada la validación de serie duplicada en `backend/routes/equipos.js` (POST y PUT) y la alerta de serie duplicada en `EquipoFormulario.jsx`. La serie ya NO es única ni obligatoria (se permiten series repetidas o vacías). En su lugar, POST y PUT de `/api/equipos` rechazan con 400 si Equipo, Marca o Modelo vienen vacíos (`"Complete Equipo, Marca y Modelo antes de guardar"`), y `EquipoFormulario.jsx` alerta antes de guardar si falta alguno. `Equipos.jsx` muestra `err.response?.data?.msg` en vez de "Error al guardar" genérico. |
| 2.00 | 20 Agosto 2026 | **Quitado el mensaje de validación del botón "Registrar en Equipos" en el form OT** (`frontend/src/pages/OrdenTrabajo.jsx`): eliminada la alerta "Complete Equipo, Marca y Modelo antes de registrar en Equipos" del `registrarEquipoEnMantenedor()`. El botón ahora intenta registrar directo sin validar en frontend (el backend igual rechaza con su propio mensaje si faltan datos). Solo frontend |
| 2.01 | 20 Agosto 2026 | **Botón "Registrar en Equipos" alineado con los buscadores de serie y modelo** (`frontend/src/components/ordenes/OrdenFormEquipo.jsx`): el botón ya no queda debajo de los buscadores; ahora es el tercer elemento de la misma fila (`alignSelf: flex-end`, height 24px) junto a "Buscar Equipo por Serie" y "Buscar por Modelo", ambos inputs a `width: 100%` de su columna. Solo frontend |
| 2.02 | 20 Agosto 2026 | **Orden de secciones en form OT** (`frontend/src/components/ordenes/OrdenFormCliente.jsx`): la sección "Adjunto" ahora va **antes** de "Información Interna" (antes era al revés). El formulario queda: Datos del Cliente → Otros Contactos → Adjunto → Información Interna. Solo frontend |
| 2.03 | 20 Agosto 2026 | **Adjuntar imagen o PDF en OT** (`frontend/src/components/ordenes/OrdenFormCliente.jsx`): el input de adjuntos ahora acepta `image/*,.pdf` (antes solo imágenes). El filtro valida tipos `image/` y `application/pdf` (máx. 2 archivos, 5MB c/u). En la lista, el PDF muestra un icono `FileText` rojo en vez de miniatura; las imágenes siguen con thumbnail. El modal "Ver" muestra el PDF en un `<iframe>` a tamaño cómodo de lectura (90vw × 88vh, máx 1000px) y las imágenes como antes. Labels del botón: "Seleccionar archivo" (tanto vacío como con un archivo). Sin cambios de BD ni backend. Solo frontend |
| 2.04 | 20 Agosto 2026 | **Botón "Imprimir" para imágenes adjuntas en el modal "Ver" de OT** (`frontend/src/components/ordenes/OrdenFormCliente.jsx`): al ver una imagen adjunta ahora aparece un botón verde "Imprimir" (icono `Printer`, fondo `#0D9488`) junto a "Descargar", igualando las opciones que ofrece el visor nativo de PDF del navegador (el menú "Guardar en OneDrive / Descargar / Imprimir" que se ve en el PDF es parte del visor de Chrome/Edge, no de la app). Implementación: función `imprimirAdjunto()` crea un `<iframe>` oculto con la imagen y dispara `window.print()` en su `onload`; no aplica a PDFs (ya tienen Imprimir en su barra nativa). Solo frontend |
| 2.05 | 20 Agosto 2026 | **Eliminar cliente solo para admin** (`frontend/src/components/clientes/ClienteAcciones.jsx` + `backend/routes/clientes.js` ambas ramas): el botón "Eliminar" del menú `...` de la tabla de Clientes ahora solo se muestra si `rol === 'admin'` (vía `parseToken()` de helpers); los usuarios con perfil técnico ya no lo ven. Respaldo en backend: `DELETE /api/clientes/:id` ahora requiere `authMiddleware + adminOnly` (antes solo `authMiddleware`), así que un técnico no puede eliminar por API aunque llame al endpoint directo. Sin cambios de BD |
| 2.06 | 21 Agosto 2026 | **Botón "Registrar en Equipos" también al editar OT** (`frontend/src/components/ordenes/OrdenFormEquipo.jsx`): el botón verde "+ Registrar en Equipos" (que estaba visible **solo al crear una OT nueva**, condición `!readOnly && !editingId`) ahora también aparece al **editar** una OT — la condición pasó a `!readOnly`, con el mismo estilo visual (verde, altura 24px, alineado junto a los buscadores de serie/modelo). Sigue oculto en modo "Ver". La función `registrarEquipoEnMantenedor()` en `OrdenTrabajo.jsx` ya era compatible (toma equipo/marca/modelo/serie del form y hace POST a `/api/equipos`). Comentario actualizado en `OrdenTrabajo.jsx`. Solo frontend, sin cambios de BD |
| 2.07 | 21 Agosto 2026 | **Menú `...` de adjuntos visible y corregido en modo "Ver" de OT** (`frontend/src/components/ordenes/OrdenFormCliente.jsx`): (1) El botón `...` de cada adjunto estaba envuelto en `{!readOnly && ...}` y no aparecía en modo Ver — ahora se muestra **siempre**, pero el menú solo contiene la opción **"Ver"** en lectura; "Descargar" y "Eliminar" siguen solo en Nueva/Editar (envueltos en `{!readOnly && <>...</>}`). Al abrir desde Ver se ve la imagen o el PDF en el modal como siempre. (2) Fix posición del menú: se calculaba con `getBoundingClientRect()` en el momento del clic y salía desfasada ("en otro lado"). Ahora usa el patrón v1.57 de los menús de tablas: `btnAdjuntoRef` guarda el botón clickeado y un `useLayoutEffect` sobre `[mostrarMenuAdjunto]` mide botón + altura real del menú después de renderizar, lo alinea justo bajo el botón (`rect.right - 140` clampeado a viewport), lo abre hacia arriba si no hay espacio abajo y nunca sale del borde. Solo frontend, sin cambios de BD |
| 2.08 | 21 Agosto 2026 | **Campo Serie eliminado del mantenedor de Equipos — la serie ahora vive solo en la OT** (`frontend/src/components/equipos/EquipoFormulario.jsx`): el formulario de Nuevo/Editar/Ver equipo ya no muestra el input Serie; quedan **Código + Equipo | Marca | Modelo** en una sola fila `.ef-r3` (antes 2 filas `.ef-r2` con Serie). El estado `serie` se mantiene internamente y sigue viajando en POST/PUT sin cambios: las series ya guardadas en la BD **se preservan** al editar (solo dejan de ser visibles/editables), y los equipos nuevos se crean con serie NULL. La serie sigue plenamente funcional en la OT ("Datos del Equipo" + buscador por serie en `OrdenFormEquipo.jsx`). Recordar: la serie ya estaba oculta en tabla/tarjetas/filtros del mantenedor desde v1.32. Backend intacto (serie opcional en POST/PUT). Sin migración de BD |
| 2.09 | 21 Agosto 2026 | **No crear equipos duplicados al "Registrar en Equipos" desde la OT** (`backend/routes/equipos.js`): el `POST /api/equipos` ahora verifica antes de insertar si ya existe un equipo con la **misma combinación Equipo + Marca + Modelo** (`LOWER(TRIM(...))` en los 3 campos, case-insensitive e ignorando espacios) → si existe responde **400** con `"El equipo X MARCA MODELO ya existe en el mantenedor con el código EQ-XXXX. No se creó un nuevo registro."` y no inserta nada; si no existe lo crea como siempre. La validación está en el backend, así que cubre tanto el botón verde "+ Registrar en Equipos" del form OT (que muestra el mensaje vía `err.response?.data?.msg`) como la creación directa desde el mantenedor de Equipos. El PUT no se toca (editar un equipo sigue permitido sin check de duplicados). Sin migración de BD |
| 2.10 | 21 Agosto 2026 | **Botón "Editar" (lápiz) para editar el equipo del mantenedor desde la OT** (`frontend/src/components/ordenes/OrdenFormEquipo.jsx` + `frontend/src/pages/OrdenTrabajo.jsx` + `frontend/src/components/equipos/EquipoFormulario.jsx`, solo frontend): (1) Nuevo botón ámbar "Editar" en la fila de buscadores de "Datos del Equipo" (misma línea que Buscar por Serie / Buscar por Modelo / "+ Registrar en Equipos"), visible solo cuando la combinación escrita en Equipo + Marca + Modelo coincide con un equipo **existente** del mantenedor (check en vivo contra el array `equipos`, `trim().toLowerCase()`; con datos ficticios no aparece). Al clickear abre modal inline editable (`EquipoFormulario`) con ese equipo exacto. (2) `handleGuardarEdicionEquipo`: PUT → refresca lista vía nueva prop `onEquiposRefresh={(lista) => setEquipos(lista)}` → **sincroniza los campos de la OT** (equipo/marca/modelo/serie) con los datos frescos; "Guardar Cambios" re-consulta el equipo y deja el modal abierto, "Cerrar" guarda y cierra el modal quedando en la OT. (3) **Fix crítico**: los modales Ver/Editar equipo se renderizaban dentro del `<form>` de la OT y el submit del botón "Cerrar" (`type="submit"`) burbujeaba hasta `onSubmit={guardarOrden}` → guardaba la OT y expulsaba al listado. Solución doble: `e.stopPropagation()` en `EquipoFormulario.handleSubmit` + modales renderizados con `createPortal(..., document.body)` (fuera física del form). (4) Label del buscador acortado: "Buscar Equipo por Serie" → "Buscar por Serie". Sin cambios de backend ni BD (idéntico en ambas ramas) |
| 2.11 | 21 Agosto 2026 | **Paginación de Clientes persistente al editar/cancelar** (`frontend/src/pages/Clientes.jsx`, solo frontend): reporte del usuario — estando en la página 2 del mantenedor, Editar un cliente y Cancelar lo devolvía a la página 1. La prueba automatizada (Playwright contra producción) no reprodujo el bug (página 2 se conservaba en Cancelar y en X), pero se agregó un arreglo defensivo definitivo: `paginaActual` ahora se **persisten en `sessionStorage`** (clave `pagClientes`) — se restaura al montar el componente y se guarda en cada cambio, así la página se mantiene al abrir/cerrar el formulario, al recargar con F5 o al volver de otra pantalla. Solo vuelve a la página 1 al cambiar los filtros (Razón Social / RUT), como siempre. Además un efecto de seguridad ajusta la página si los datos se reducen (ej: eliminados) para no quedar en una página fuera de rango. Sin cambios de backend ni BD |
| 2.12 | 23 Agosto 2026 | **Buscador por Serie eliminado de "Datos del Equipo" en la OT + script de migración de columnas**: (1) `OrdenFormEquipo.jsx`: removido el bloque "Buscar por Serie" — los equipos ahora solo se buscan **por Modelo** (el mantenedor de Equipos ya no gestiona series desde v2.08, así que buscar por serie no tenía sentido). Quedan en la fila: "Buscar por Modelo" + botones "+ Registrar en Equipos", "Editar" (ámbar) y "Ver". El campo **Serie** sigue existiendo como input manual en la grilla Equipo/Marca/Modelo (se escribe a mano o queda vacío). La fila del buscador ahora usa la misma grilla `.of-r3` (3 columnas, gap 20px) que Equipo/Marca/Modelo → el buscador queda exactamente con el mismo ancho que el input "Equipo \*"; botones con `alignSelf:'end'`. Bonus mobile ≤768px: `.of-r3` apila a 1 columna (buscador full-width, botones debajo). (2) `OrdenTrabajo.jsx`: limpieza completa — eliminados estado `busquedaSerie`, estado `mostrarDropdownEquipos`, estado `equiposSugeridos`, ref `equipoDropdownRef`, el `useEffect` de búsqueda por serie vía API, la función `seleccionarEquipo()` (por serie), todos los `setBusquedaSerie("")`/`setEquiposSugeridos([])` en resets y los props relacionados en el JSX. Solo frontend, sin cambios de BD. (3) Nuevo `scripts/migrar-columnas-faltantes.js`: verifica contra `information_schema` y agrega columnas faltantes en la BD local MySQL usando el `.env` de `backend/` (resuelve módulos desde `backend/node_modules`). Se usó para aplicar la migración pendiente de v1.94 (`ALTER TABLE ordenes_trabajo ADD COLUMN adjunto LONGTEXT`) que causaba `ER_BAD_FIELD_ERROR: Unknown column 'adjunto'` al guardar OT. Ejecutar desde `backend/`: `node ..\scripts\migrar-columnas-faltantes.js` |
| 2.13 | 23 Agosto 2026 | **Búsqueda de cliente por RUT con puntos automáticos + match por dígitos**: (1) **Problema**: al buscar un cliente por RUT en la OT escribiendo `14900` no encontraba nada porque el RUT guardado es `14.900.665-6` y el filtro comparaba el texto crudo. (2) **Match por dígitos**: las comparaciones ahora extraen solo los dígitos (`replace(/[^0-9]/g, "")`) — así buscar `14900`, `14.900` o el RUT completo da el mismo resultado. En la OT (`OrdenTrabajo.jsx` `clientesFiltrados`) usa `includes` sobre los dígitos (matchea en cualquier parte); en el mantenedor de Clientes (`Clientes.jsx` `matchRut`) mantiene la semántica `startsWith` de siempre pero sobre dígitos. (3) **Puntos automáticos al escribir**: nuevo helper compartido `formatearRutInput(valor)` en `utils/helpers.js` — formatea progresivamente con puntos mientras se escribe (igual que el campo RUT del formulario de nuevo cliente): `14900665` → `14.900.665`; el guion y dígito verificador los escribe el usuario si quiere; máximo 12 caracteres; solo permite `[0-9K-]`. Aplicado en: buscador de cliente de la OT (`OrdenFormCliente.jsx` — solo cuando lo escrito parece RUT `/^[0-9][0-9Kk.-]*$/`, si contiene letras sigue funcionando como texto normal para razón social/código) y filtro RUT del mantenedor de Clientes (`FiltrosCliente.jsx` — reemplaza el `formatearRut` local duplicado). (4) **Refactor helpers.js**: nueva función exportada `calcularDV(cuerpo)` (módulo 11) reutilizada por `validarRUT()` (eliminada la lógica duplicada del cálculo interno). (5) **Descartado**: una versión intermedia auto-completaba el guion+DV calculado automáticamente en el buscador — causaba que fuera imposible borrar (el DV se regeneraba) y ensuciaba la query; se revirtió a pedido del usuario. Solo frontend, sin cambios de BD ni backend |
| 2.14 | 23 Agosto 2026 | **Fix RUT no se autocompletaba al seleccionar cliente en OT nueva** (`frontend/src/pages/OrdenTrabajo.jsx`, solo frontend): reporte del usuario — al buscar un cliente en el buscador de la OT y seleccionarlo del dropdown, el campo RUT quedaba vacío. Causa: `seleccionarCliente()` tiene dos ramas de `setNuevaOrden` — la rama de **edición** sí incluía `rut: cliente.rut \|\| ""` (agregado en v1.76), pero la rama de **nueva orden/cambio de cliente** nunca lo traía. Fix: agregado `rut: cliente.rut \|\| ""` al segundo bloque; aplica tanto al seleccionar cliente en una OT nueva como al cambiar de cliente. Sin cambios de backend ni BD |
| 2.15 | 23 Agosto 2026 | **Editar y Registrar cliente completo desde la OT + sincronización de datos** (`frontend/src/components/ordenes/OrdenFormCliente.jsx` + `frontend/src/components/clientes/ClienteFormulario.jsx`, solo frontend): (1) Botón **"Editar"** (ámbar `var(--warning)`, icono Pencil, mismo estilo que Editar equipo: fontWeight 600, padding 2px 10px, height 24px) junto al buscador de cliente, visible cuando hay `clienteSeleccionado` y no es readOnly → abre `ClienteFormulario` editable en `createPortal(document.body)` con datos frescos vía `GET /api/clientes/:id`. Handlers `abrirGuardarEdicionCliente()`: PUT, refresca lista vía nuevo prop `onClientesRefresh`, actualiza `clienteSeleccionado` y **sincroniza los campos de la OT** (cliente/rut/direccion/comuna/email/fonoPrincipal; email/fono del contacto solo si matchea por nombre). "Guardar Cambios" reconsulta y deja abierto; "Cerrar" guarda y cierra quedando en la OT. (2) Botón **"+ Registrar en Clientes"** (verde, UserPlus) junto al buscador cuando NO hay cliente seleccionado pero hay texto en Cliente o RUT → abre modal `ClienteFormulario` precargado con los datos escritos en la OT (nuevo prop `modoRegistro`: oculta "Guardar Cambios", botón primario dice "Guardar Cliente"). Si el RUT o la razón social ya existen → alerta con su código CL-XXXX, limpia el campo RUT y NO abre el formulario. `guardarNuevoClienteDesdeOT()`: chequeo local previo por dígitos de RUT / razón social normalizada (alerta con código sin crear duplicado); si no existe hace POST `/api/clientes`, refresca lista, vincula por `res.data.id` y sincroniza campos. (3) Backend (`backend/routes/clientes.js`): mensajes de RUT duplicado ahora incluyen el código — POST `"El cliente ya existe en el mantenedor con el código CL-XXXX. No se creó un nuevo registro."` y PUT `"El RUT ya existe..."`; también en `ClienteFormulario.jsx` (chequeo local con `.find`). Sin migración de BD |
| 2.16 | 23 Agosto 2026 | **Guardar Orden exige cliente existente y seleccionado del buscador** (`frontend/src/pages/OrdenTrabajo.jsx`, solo frontend): reglas definidas con el usuario — "Guardar Orden es para guardar la orden, pero el cliente tiene que estar seleccionado en el Buscar Cliente; el botón Registrar es para registrar clientes". Validaciones en `guardarOrden` en este orden: (1) Cliente obligatorio, (2) RUT obligatorio + dígito verificador válido (`validarRUT`), (3) existencia en mantenedor por razón social O dígitos de RUT → si no: `El cliente no está registrado. Use "Registrar en Clientes".`, (4) **selección explícita**: sin `clienteSeleccionado` no se guarda aunque los datos coincidan con un cliente existente → `Seleccione un cliente del buscador antes de guardar.`, (5) conflicto de RUT: si pertenece a otro cliente distinto del seleccionado → `El RUT X pertenece al cliente CL-XXXX.`. Eliminada la auto-vinculación silenciosa que existed antes (vinculaba sin pasar por el buscador). Mensajes cortos a pedido del usuario |
| 2.17 | 23 Agosto 2026 | **Equipo seleccionado automáticamente al editar/ver OT + limpieza visual de "Datos del Equipo"** (`frontend/src/pages/OrdenTrabajo.jsx` + `frontend/src/components/ordenes/OrdenFormEquipo.jsx` + `OrdenFormCliente.jsx`, solo frontend): (1) `editarOrden()` y `verOrden()` ahora buscan en el array local `equipos` un registro cuya combinación Equipo+Marca+Modelo coincida (normalizado trim+uppercase) con el snapshot de la OT y lo setean como `equipoSeleccionado` — igual que se hace con el cliente — para que el botón "Editar"/"Ver" esté disponible sin re-escribir nada (las OT no guardan `equipo_id` desde v1.34, por eso el match es por datos). (2) Fondo del input buscador de equipo cuando hay selección: `#E0F2FE` (azul claro igual que el buscador de cliente; antes verde `#DCFCE7`). (3) Eliminado el banner verde "✓ Seleccionado: ..." que aparecía bajo el buscador al editar (a pedido del usuario; la selección se nota por el fondo del input y el botón Editar). Quedó solo el badge legacy de `equipoFijo`. (4) Fix botón "Ver" duplicado en modo Ver: había uno arriba (header de la sección) y otro en la fila de buscadores — eliminado el de arriba, queda solo el de la fila. (5) Los 3 botones "Ver" del form OT (cliente ×2 y equipo) subieron de `fontWeight: 500` a `600` para igualar el grosor de letra de "Registrar en Equipos" y "Editar". Build OK |

> ⚠️ **NOTA REVERSIÓN v1.98 (si el cliente no lo quiere):** el commit a revertir es `72341c2` (solo `main`, no fue a `deploy/cloud`). La versión anterior sin el botón es `aef95e4`. Para volver atrás: `git revert 72341c2` (o `git reset --hard aef95e4` si no se commiteó nada después). Eso restaura la OT sin el botón pero **conserva** la validación de campos obligatorios del mantenedor (v1.97, commit `aef95e4`). Archivos que se revierten: `frontend/src/pages/OrdenTrabajo.jsx` (función `registrarEquipoEnMantenedor` + prop `onRegistrarEquipo`) y `frontend/src/components/ordenes/OrdenFormEquipo.jsx` (botón + props `editingId`/`onRegistrarEquipo`). No hay cambios de BD ni de backend.
