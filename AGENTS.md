...
| 1.97 | 20 Agosto 2026 | **OT registra equipos en el mantenedor solo si no existen** (`backend/routes/ordenes.js` — solo `main`): nueva función `registrarEquipoSiNoExiste()` ejecutada en POST y PUT de OT. Si la serie escrita en la OT **no existe** en la tabla `equipos`, crea el registro copiando **solo identidad** (equipo, marca, modelo, serie) con código `EQ-XXXX` auto-generado. Si la serie **ya existe** → no duplica. **Sin serie** → no crea (la serie identifica la máquina física). Los demás datos de la OT (contador, nivel tinta, avería, actividad, observaciones, insumos) son propios de la OT y NO se copian al mantenedor. El equipo NO se vincula a la OT (`equipo_id` sigue NULL) ni a cliente. Verificado localmente (3 casos: serie nueva crea, serie existente no duplica, sin serie no crea) |
